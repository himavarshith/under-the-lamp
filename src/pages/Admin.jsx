...
import {
  Shield, Users, ArrowUp, ArrowDown, Trash2, Send, Upload, Plus,
  BookOpen, Camera, CheckCircle, XCircle, Clock, Eye, EyeOff
} from 'lucide-react'
...
// ----- Demo waitlist data -----
const DEMO_WAITLIST = [
  { id: '1', name: 'Amara Johnson', email: 'amara@email.com', phone: '555-0101', position: 1, status: 'waiting', created_at: '2025-12-15' },
  { id: '2', name: 'Chen Wei', email: 'chen@email.com', phone: '555-0102', position: 2, status: 'waiting', created_at: '2025-12-20' },
  { id: '3', name: 'Priya Patel', email: 'priya@email.com', phone: '555-0103', position: 3, status: 'invited', created_at: '2026-01-02' },
  { id: '4', name: 'Luca Rossi', email: 'luca@email.com', phone: null, position: 4, status: 'accepted', created_at: '2026-01-10' },
  { id: '5', name: 'Sofia Martinez', email: 'sofia@email.com', phone: '555-0105', position: 5, status: 'waiting', created_at: '2026-01-15' },
  { id: '6', name: 'Kwame Asante', email: 'kwame@email.com', phone: '555-0106', position: 6, status: 'declined', created_at: '2026-02-01' },
  { id: '7', name: 'Yuki Tanaka', email: 'yuki@email.com', phone: null, position: 7, status: 'waiting', created_at: '2026-02-10' },
]

const statusConfig = {
  waiting: { label: 'Waiting', color: 'bg-warm-100 text-warm-600', icon: Clock },
  invited: { label: 'Invited', color: 'bg-blue-50 text-blue-600', icon: Send },
  accepted: { label: 'Accepted', color: 'bg-green-50 text-green-600', icon: CheckCircle },
  declined: { label: 'Declined', color: 'bg-red-50 text-red-600', icon: XCircle },
  expired: { label: 'Expired', color: 'bg-warm-100 text-warm-400', icon: Clock },
}
...
function AdminLogin({ onLogin }) {
  ...
  return (
    <div className="max-w-sm mx-auto px-4 py-32 text-center">
      ...
      <p className="text-warm-400 text-sm mb-6">Enter the admin password to continue.</p>
...
function WaitlistTab() {
  ...
  const triggerInvite = async (item) => {
    ...
    setWaitlist(
      waitlist.map((w) =>
        w.id === item.id ? { ...w, status: 'invited' } : w
      )
    )
  }
  ...
  const triggerMonthlyRound = async () => {
    ...
    if (waiting.length === 0) {
      alert('No one in the waiting queue!')
      return
    }

    if (!confirm(`Send invitations to the top ${Math.min(4, waiting.length)} people on the waitlist?`)) return

    setSending(true)
    try {
      // Call the Supabase Edge Function
      const { data, error } = await supabase.functions.invoke('send-monthly-invites')

      if (error) throw error

      // Refresh local state
      const toInvite = waiting.slice(0, 4)
      setWaitlist(
        waitlist.map((w) =>
          toInvite.find((t) => t.id === w.id) ? { ...w, status: 'invited' } : w
        )
      )
      alert(`✅ Sent ${data?.invited || toInvite.length} invitation(s)!`)
    } catch (err) {
      // Fallback: update UI only (for demo / when edge function isn't deployed)
      const toInvite = waiting.slice(0, 4)
      setWaitlist(
        waitlist.map((w) =>
          toInvite.find((t) => t.id === w.id) ? { ...w, status: 'invited' } : w
      ...
      alert(`📧 Invites marked locally (edge function not available: ${err.message})`)
    } finally {
      setSending(false)
    }
  }
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          <div key={s.label} className="bg-white rounded-xl p-4 border border-warm-100 text-center">
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="font-medium text-sm text-warm-900 truncate">{item.name}</p>
      ...
      <div className="bg-white rounded-xl p-6 border border-warm-100">
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <p className="text-xs text-warm-300 mt-1">PNG, JPG, WEBP up to 10MB each</p>
            <input
              type="file"
              multiple
              accept="image/*"
